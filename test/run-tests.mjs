...
test('service worker precaches the versioned install manifest and every app icon', serviceWorker.includes('./manifest.webmanifest?v=15') && webManifest.icons.every((icon) => serviceWorker.includes(`./${icon.src}`)) && serviceWorker.includes('./icons/apple-touch-icon.png'));
...
test('service worker shell cache is v12', serviceWorker.includes("const SHELL_CACHE = `${CACHE_PREFIX}shell-v15`"));
...
test('HTML links the versioned manifest and explicit Apple touch icon', html.includes('rel="manifest" href="manifest.webmanifest?v=15"') && html.includes('rel="apple-touch-icon"') && html.includes('sizes="180x180"'));
...
test('English UI dictionary is loaded dynamically on demand', i18nSource.includes("import('../locales/en.js?v=15')") && !serviceWorker.includes('./locales/en.js?v=15'));
test('Persian UI, country data, and preference modules are available in the offline shell', ['./locales/fa.js?v=15', './data/countries.json?v=15', './js/preferences.js?v=15'].every((asset) => serviceWorker.includes(asset)));
...
test('assessment topbar shows a single labelled home chip, no separate back icon', assessmentSource.includes('back-home-chip') && assessmentSource.includes("t('home')") && !assessmentSource.includes('id="back-step"') && !assessmentSource.includes('topbar-nav'));
...
test('versioned v11 shell assets bypass an older cache during this upgrade', html.includes('css/app.css?v=15') && html.includes('js/app.js?v=15') && serviceWorker.includes('./css/app.css?v=15') && serviceWorker.includes('./js/app.js?v=15'));
test('every browser module dependency is versioned together', appSource.includes("'./kb.js?v=15'") && appSource.includes("'./engine.js?v=15'") && kbSource.includes("'./schema.js?v=15'") && serviceWorker.includes('./js/kb.js?v=15') && serviceWorker.includes('./js/engine.js?v=15') && serviceWorker.includes('./js/schema.js?v=15'));
...
test('registration bypasses HTTP cache when checking the worker', appSource.includes("updateViaCache: 'none'") && appSource.includes("register('./sw.js?v=15'"));
